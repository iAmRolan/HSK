...
{ 
		public class AdditionAbsences_CanReportedUntilEndOfMonth : IWebTest
		{
			...
			public ExecutionResult Execute(WebTestHelper helper)
			{
				...
				var Warning_Msg_Idkun = driver.TestProject().GetText(by);
				report.Step("Get text from 'היעדרויות שורת הערה'", Warning_Msg_Idkun != null, TakeScreenshotConditionType.Always);
				driver.Timeout = 15000;
				...
				if (!Warning_Msg_Idkun.Equals("היעדרות מסוג זה לא ניתן לדווח מעבר לחודש שוטף."))
				{
					// Add step sleep time (Before)
					Thread.Sleep(500);
